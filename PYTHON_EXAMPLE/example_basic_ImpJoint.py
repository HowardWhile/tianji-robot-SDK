"""
@file example_basic_ImpJoint.py
@brief Example demonstrating how to control the L1 robot arm
       in joint-space impedance control mode (STATE_IMP_JOINT).

Workflow overview:
    1. Initialize communication with the robot controller
    2. Retrieve SDK and controller versions
    3. Switch ARM into impedance control mode (STATE_IMP_JOINT)(Check and recover the arm state (ERROR → IDLE))
...
def main():
    ...
    print(f"\n### 3/6. Impedance joint state switching...")
    ...
    while 1:
        ...
        if robot.check_sequences_approx_equal(fb_joints, pos1,tolerance=0.5):
            break
        time.sleep(0.001)
    ...
    while 1:
        rt_dict = robot.get_rt_dict()
        fb_joints = rt_dict["arms"][arm_idx]["fb"]["fb_pos"]
        if robot.check_sequences_approx_equal(fb_joints, pos2,tolerance=0.5):
            break
        time.sleep(0.001)
    print(f"Arm0 reached at {fb_joints}")
