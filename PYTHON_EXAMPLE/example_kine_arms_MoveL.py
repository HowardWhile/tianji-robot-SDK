"""
@file example_kine_arms_MoveL.py
...
def main():
    ...
    pln_acc=100
    pln_freq=50
    arm0_start_xyzabc = [509.734, 233.609, 365.948, -169.144, 55.011, -146.752]
    arm1_start_xyzabc = [509.734, -233.609, 365.948, 169.144, 55.011, 146.752]

    arm0_start_pos=[19.597, -32.480, 10.050, -58.939, -8.863, -33.821, 4.772]
    arm1_start_pos=[-19.597,-32.480,-10.050,-58.939,8.863,-33.821,-4.772]

    arm0_end_xyzabc=[509.734, 233.609, 265.948, -169.144, 55.011, -146.752]
    arm1_end_xyzabc = [509.734, -233.609, 265.948, 169.144, 55.011, 146.752]
    ...
    while 1:
        ...
        if robot.check_sequences_approx_equal(fb_joints1, arm0_start_pos) and robot.check_sequences_approx_equal(fb_joints2, arm1_start_pos):
            break
        time.sleep(0.001)
    ...
    points0 = robot.plan_linear(0, arm0_start_xyzabc, arm0_end_xyzabc, arm0_start_pos, pln_vel, pln_acc, pln_freq)
    ...
    points1 = robot.plan_linear(1, arm1_start_xyzabc, arm1_end_xyzabc, arm1_start_pos, pln_vel, pln_acc, pln_freq)
