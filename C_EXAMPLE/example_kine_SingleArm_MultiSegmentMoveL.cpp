/**
 * @file example_kine_SingleArm_MultiSegmentMoveL.cpp
 * @brief Example demonstrating two single-arm multi-segment MoveL test functions.
 *
 * This example provides:
 * - `SingleArm_MultiSegmentMoveL_ByIniConfig()`: keeps the original controller-linked
 *   workflow and initializes ARM0 with `FX_L1_Kinematics_InitSingleArm_ByIniConfig`
 * - `SingleArm_MultiSegmentMoveL_ByInputParams()`: does not link to the controller and
 *   only performs the multi-segment MoveL planning function after
...
/**
 * @brief Run the controller-linked single-arm multi-segment MoveL example.
 *
 ...
 * 5. Plan the multi-segment MoveL trajectory and execute it on the controller
 *
 * @return int Exit code (0 on normal exit)
 */
int SingleArm_MultiSegmentMoveL_ByIniConfig()
{
    /* Parameters declaration. */
    int sdk_version = 0;
    ...
    double arm0_start_joints[7] = {17.970, -35.197, 11.414, -73.344, -9.154, -17.035, 7.086};
    double arm0_start_matrix[4][4] = {{0}};
    double arm0_start_xyzabc[6] = {0};
    double arm0_next_xyzabc[6] = {0};
    double allow_error_ = 5.0;
    double arm0_pln_vel = 100.0;
    double arm0_pln_acc = 500.0;
    double arm0_zsp_para[6] = {0, 0, -1, 0, 0, 0};
    int arm0_zsp_type = 1;
    int arm0_pln_freq = 50;
    ...
    FX_L1_Matrix2XYZABC(arm0_start_matrix, arm0_start_xyzabc);
    for (index = 0; index < 6; ++index)
    {
        arm0_next_xyzabc[index] = arm0_start_xyzabc[index];
    }

    /* Plan the Multi_Points trajectories for Arm0. */
    arm0_next_xyzabc[2] += 200.0;
    err_code = FX_L1_Kinematics_PlanLinearMove_MultiPoints_SetStart(handle, 0, arm0_start_joints, arm0_start_xyzabc, arm0_next_xyzabc,
                                                                    allow_error_, arm0_zsp_type, arm0_zsp_para,
                                                                    arm0_pln_vel, arm0_pln_acc, arm0_pln_freq);
    if (err_code != FUNC_RET_SUCCESS)
    {
        printf("Failed to set ARM0 start trajectory. Error code: %d\n", err_code);
        goto WAIT_EXIT;
    }

    arm0_next_xyzabc[1] -= 200.0;
    err_code = FX_L1_Kinematics_PlanLinearMove_MultiPoints_SetNextPoints(handle, 0, arm0_next_xyzabc, allow_error_, arm0_zsp_type, arm0_zsp_para,
                                                                         arm0_pln_vel, arm0_pln_acc);
    if (err_code != FUNC_RET_SUCCESS)
    {
        printf("Failed to plan multi-segment MoveL segment 2. Error code: %d\n", err_code);
        return -1;
    }
    arm0_next_xyzabc[2] -= 200.0;
    err_code = FX_L1_Kinematics_PlanLinearMove_MultiPoints_SetNextPoints(handle, 0, arm0_next_xyzabc, allow_error_, arm0_zsp_type, arm0_zsp_para,
    ...
    if (err_code != FUNC_RET_SUCCESS)
    {
        printf("Failed to plan multi-segment MoveL segment 3. Error code: %d\n", err_code);
        return -1;
    }
    arm0_next_xyzabc[1] += 200.0;
    err_code = FX_L1_Kinematics_PlanLinearMove_MultiPoints_SetNextPoints(handle, 0, arm0_next_xyzabc, allow_error_, arm0_zsp_type, arm0_zsp_para,
    ...
    if (err_code != FUNC_RET_SUCCESS)
    {
        printf("Failed to plan multi-segment MoveL segment 4. Error code: %d\n", err_code);
        return -1;
    }
    err_code = FX_L1_Kinematics_PlanLinearMove_MultiPoints_GetPoints(handle, arm0_planned_points, &point_num);
    if (err_code != FUNC_RET_SUCCESS)
    {
        printf("Failed to export multi-segment MoveL trajectory points. Error code: %d\n", err_code);
        return -1;
    }
    printf("Plan ARM0 multi-segment MoveL trajectory success, points = %d\n", point_num);
...
/**
 * @brief Run the controller-free single-arm multi-segment MoveL example.
 *
 * The workflow includes:
 * 1. Query the SDK version
 * 2. Create the kinematics context
 * 3. Initialize ARM0 kinematics from input parameters
 * 4. Plan the multi-segment MoveL trajectory without executing it on the controller
 *
 * @return int Exit code (0 on normal exit)
 */
int SingleArm_MultiSegmentMoveL_ByInputParams()
{
    /* Parameters declaration. */
    int point_num = 0;
    int err_code = 0;
    ...
    /* Build the standalone ARM0 kinematics model directly from hard-coded geometric parameters. */
 ...
 * `SingleArm_MultiSegmentMoveL_ByIniConfig()`.
 * Replace it with `SingleArm_MultiSegmentMoveL_ByInputParams()` when only the
...
int main(int argc, char **argv)
{
    ...
    return SingleArm_MultiSegmentMoveL_ByIniConfig();
    //return SingleArm_MultiSegmentMoveL_ByInputParams();
}