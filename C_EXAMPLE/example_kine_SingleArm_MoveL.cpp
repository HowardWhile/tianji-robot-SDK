/**
 * @file example_kine_SingleArm_MoveL.cpp
 * @brief Example demonstrating two single-arm MoveL test functions.
 *
 * This example provides:
 * - `SingleArm_MoveL_ByIniConfig()`: keeps the original controller-linked
 *   workflow and initializes ARM0 with `FX_L1_Kinematics_InitSingleArm_ByIniConfig`
 * - `SingleArm_MoveL_ByInputParams()`: does not link to the controller and only
 *   performs the MoveL planning function after
...
/**
 * @brief Run the controller-linked single-arm MoveL example.
 *
 ...
 * 5. Plan the MoveL trajectory and execute it on the controller
 *
 * @return int Exit code (0 on normal exit)
 */
int SingleArm_MoveL_ByIniConfig()
{
    /* Parameters declaration. */
    int sdk_version = 0;
    ...
    printf("Plan ARM0 MoveL trajectory success, points = %d\n", point_num);
...
/**
 * @brief Run the controller-free single-arm MoveL example.
 *
 * The workflow includes:
 * 1. Query the SDK version
 * 2. Create the kinematics context
 * 3. Initialize ARM0 kinematics from input parameters
 * 4. Plan the MoveL trajectory without executing it on the controller
 *
 * @return int Exit code (0 on normal exit)
 */
int SingleArm_MoveL_ByInputParams()
{
    /* Parameters declaration. */
    int point_num = 0;
    int err_code = 0;
 ...
 * `SingleArm_MoveL_ByIniConfig()`.
 * Replace it with `SingleArm_MoveL_ByInputParams()` when only the
